            ...
            default:
                {
                    ...
                    var assetInfo =
                        await _binanceMainClient.BinanceOrderService.GetExchangeInfoFutures(model.IndicatorHook?.Order?
                            .Symbol ?? "", cancellationToken);
                    if (model.LossBasedPlan?.LossAmountPercentage != null)
                    {
                        var marketPrice =
                            await _binanceMainClient.BinanceOrderService.GetMarketPriceForFutures(
                                model.IndicatorHook?.Order?.Symbol ?? "", cancellationToken);
                        var lossAmount = model.CurrentAdjustedBalance * model.LossBasedPlan.LossAmountPercentage / 100;
                        var stopAmount = side is Side.Long
                            ? marketPrice- Math.Round(marketPrice - marketPrice *
                                (model.LossBasedPlan.StopLossPercentage is null
                                    ? model.IndicatorHook?.StopLossPercentage ?? 1
                                    : model.LossBasedPlan?.StopLossPercentage ?? 1) / 100, 2)
                            :  Math.Round(marketPrice + marketPrice *
                                (model.LossBasedPlan.StopLossPercentage is null
                                    ? model.IndicatorHook?.StopLossPercentage ?? 1
                                    : model.LossBasedPlan?.StopLossPercentage ?? 1) / 100, 2)-marketPrice;

                        var leverage =  (int)(lossAmount / stopAmount * marketPrice / model.CurrentAdjustedBalance);

                        model.IndicatorHook!.Order!.Leverage=leverage;
                    }
                    ...
                    if (balanceAndQuantity.Item1.TotalBalance < model.MinimumBalance ||
                    balanceAndQuantity.Item1.AvailableFuturesBalance <
                    model.CurrentAdjustedBalance || balanceAndQuantity.Item2 is 0)
                        break;
                    ...
                    await SetSlAndTpAndTsl(model.IndicatorHook!.Order!.Symbol, side,
                        ...
                        model.IndicatorHook?.TakeProfitPercentage, model.IndicatorHook?.StopLossPercentage??model.LossBasedPlan?.StopLossPercentage??null,
                    ...
                    responseDto = new HookResponseTradingHistory
                    {
                        Side = model.IndicatorHook!.Order.Side,
                        Symbol = model.IndicatorHook.Order.Symbol,
                        OrderType = model.IndicatorHook.Order.OrderType,
