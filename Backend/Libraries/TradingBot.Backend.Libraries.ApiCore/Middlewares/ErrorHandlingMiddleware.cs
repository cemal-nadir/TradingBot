...
using TradingBot.Backend.Libraries.ApiCore.Responses;
using TradingBot.Backend.Libraries.Domain.Defaults;

namespace TradingBot.Backend.Libraries.ApiCore.Middlewares;
...
public class ErrorHandlingMiddleware
{
    ...
    private static string ParseToMessage(string message)
    {
        ...
        {
            result = Error.DbException.Duplicate;
        }
        else if (message.Contains("REFERENCE constraint \"FK_"))
        {
            ...
            return $"{Error.DbException.ReferenceConstraint}|{first}";
        }
        return result;
    }
}